...
export default function DemoPage() {
  ...
  const [sidebarItems] = useState<SidebarItem[]>([
    { id: 'sidebar-1', name: 'Nueva Card 1', color: 'bg-orange-500', type: 'type1' },
    { id: 'sidebar-2', name: 'Nueva Card 2', color: 'bg-pink-500', type: 'type2' },
    { id: 'sidebar-3', name: 'Nueva Card 3', color: 'bg-teal-500', type: 'type3' },
    { id: 'sidebar-4', name: 'Nueva Card 4', color: 'bg-cyan-500', type: 'type1' },
    { id: 'sidebar-5', name: 'Nueva Card 5', color: 'bg-lime-500', type: 'type2' },
  ])
  ...
  const handleDragOver = (event: DragOverEvent) => {
    ...
    if (active && over) {
      ...
      if (activeItem && overItem && activeItem.type === overItem.type && active.id !== over.id) {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (activeItem && overItem && activeItem.type === overItem.type && active.id !== over.id) {
      ...
      if (active.id.toString().startsWith('sidebar-') && gridItems.some(item => item.id === over.id)) {
      ...
      else if (!active.id.toString().startsWith('sidebar-') && gridItems.some(item => item.id === over.id)) {
        setGridItems(items => {
      ...
      else if (active.id.toString().startsWith('sidebar-') && sidebarItems.some(item => item.id === over.id)) {
    ...
    else if (!active.id.toString().startsWith('sidebar-') && active.id !== over.id) {
      setGridItems((items) => {
        const oldIndex = items.findIndex((item) => item.id === active.id)
        const newIndex = items.findIndex((item) => item.id === over.id)
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <h1 className="text-2xl font-bold text-gray-900">Demo - Drag & Drop Grid</h1>
          <p className="text-gray-600 mt-1">
            Arrastra elementos del panel lateral al grid o reorganiza los elementos existentes
          </p>
        </div>
      </div>
        ...
        >
          <div className="flex gap-8">
            ...
            <div className="flex-1">
              <h2 className="text-lg font-semibold text-gray-900 mb-4">Grid Layout</h2>
            ...
            {/* Panel Lateral */}
            <div className="w-80">
              <h2 className="text-lg font-semibold text-gray-900 mb-4">Elementos Disponibles</h2>
              ...
              <div className="mt-6 bg-blue-50 rounded-lg p-4">
                ...
                <ul className="text-sm text-blue-800 space-y-1">
                  <li>• <strong>Mismo tipo:</strong> Borde azul = combinación</li>
                  <li>• <strong>Diferente tipo:</strong> Inserción normal</li>
                  <li>• <strong>Combinación:</strong> Se agrega a la derecha</li>
                  <li>• <strong>Reordenar:</strong> Arrastra dentro del grid</li>
                </ul>
              </div>
            </div>
          </div>
